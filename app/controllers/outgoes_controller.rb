...
class OutgoesController < ApplicationController
  ...
  def create
    ...
    if @outgo.save
      redirect_to @outgo, notice: 'Success!'
    else
  ...
  def outgo_params = params.expect(outgo: %i[date asset_id debt_id recipient_id memo]).merge(amount: amount)
end
